...
import com.example.mock_cgv.src.main.numberselect.NumberSelectActivity;
...
public class TicketingActivity extends BaseActivity implements TicketingActivityView {
    ...
    //NumberSelect Activity로 보낼 자료들
    String mTheaterName,mRoomid,mStratTime,mEndTime,mDate;
            ...
            switch (v.getId()){
                case R.id.dialog_btn_choose:
                    ...
                    if(mLoginState!=null){ //로그인 했을 때
                        ...
                        intent1.putExtra("theaterName",mTheaterName);
                        intent1.putExtra("roomid",mRoomid);
                        intent1.putExtra("stratTime",mStratTime);
                        intent1.putExtra("endTime",mEndTime);
    ...
    public void getTicketDetailSuccess(TicketDetailResponse.Result result, int movieTimeId, int code, String message) {
        Log.e("why 안되는가",""+result);
        //커스텀 다이얼로그 ㄱㄱ
        mTheaterName=result.getTheaterName();
        mRoomid=result.getRoomId();
        mStratTime=result.getStartTime();
